...
class value_weighting(nn.Module):
    """Token-wise value attention weighting (no pooling).

    This module uses the same parameterization as :class:`value_pooling`, but returns
    the weighted token representations instead of summing over the sequence length.
    ...
    def forward(self, x, mask=None, save_attention_path=None, return_attention=False):
        # (B, Len, Embed) x (Embed, Units) = (B, Len, Units)
        UV = self.U + self.V
        h = torch.matmul(x, UV)  # (B, L, Units)

        if self.use_additive_bias:
            h = torch.tanh(h + self.b1)
        else:
            h = torch.tanh(h)
        ...
        if save_attention_path is not None:
            # Save raw attention probabilities for later inspection.
            torch.save(attention_probs.detach().cpu(), save_attention_path)

        weighted_x = attention_probs * x

        if return_attention:
            return weighted_x, attention_probs
        return weighted_x
...
class residue_reweight(nn.Module):
    def __init__(self, embed_size, units=None, use_additive_bias=False, use_attention_bias=False):
        super(residue_reweight, self).__init__()
        self.embed_size = embed_size
        self.use_additive_bias = use_additive_bias
        self.use_attention_bias = use_attention_bias
        self.units = units if units else embed_size
