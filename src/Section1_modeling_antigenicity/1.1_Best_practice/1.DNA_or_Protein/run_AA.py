...
AA_data_filt1 = train_data.groupby(['seq_a', 'seq_c', 'serumPassCat', 'virusPassCat']) \
        .agg({'serumName': 'first', 'virusName': 'first', 'Type': 'first', 'label': 'mean'}) \
        .reset_index()
## remove PassCat = 'BOTH'
AA_data_filt2 = AA_data_filt1[(AA_data_filt1['serumPassCat'] != 'BOTH') &
                              (AA_data_filt1['virusPassCat'] != 'BOTH')].reset_index(drop=True)
## replace PassCat to special token
AA_data_filt3 = AA_data_filt2.replace({'serumPassCat': {'EGG': '<EGG>', 'CELL': '<CELL>'},
                                       'virusPassCat': {'EGG': '<EGG>', 'CELL': '<CELL>'}})

train_data_final = AA_data_filt3.copy()

train_dataset = AADataset(train_data_final)
valid_dataset = AADataset(valid_data)
test_dataset = AADataset(test_data)
...
early_stopping = EarlyStopping(patience=20, save_dir='/mnt/zzbnew/peixunban/chenyihao/fluProfiler_source/trained_model/1.1_AA_model/')
# Training loop
for epoch in range(num_epochs):
    ...
    ## 将epoch信息写入log.txt
    with open('/mnt/zzbnew/peixunban/chenyihao/fluProfiler_source/trained_model/1.1_AA_model/log.txt', 'a') as f:
