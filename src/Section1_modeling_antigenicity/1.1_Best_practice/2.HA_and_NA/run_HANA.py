...
def train_ddp(rank, world_size):
    ...
    train_loader = DataLoader(train_dataset, batch_size=10, sampler=train_sampler, num_workers=4)
    valid_loader = DataLoader(valid_dataset, batch_size=128, sampler=valid_sampler, num_workers=4)
    ...
    model = torch.load('/mnt/zzbnew/peixunban/chenyihao/fluProfiler_source/trained_model/1.2_HANA_model/2025-10-26_09-59-19.pth')
    model = model.to(device)
    model = DDP(model, device_ids=[rank])

    # 计数：DP时用 .module
    if rank == 0:
        print("Number of parameters: %e" % count_parameters(model.module))
    ...
    early_stopping = EarlyStopping(patience=100, save_dir='/mnt/zzbnew/peixunban/chenyihao/fluProfiler_source/trained_model/1.2_HANA_model/')
    
    # Training loop
    for epoch in range(num_epochs):
        train_sampler.set_epoch(epoch)  # 确保每个epoch的数据顺序不同
        model.train()
        loss_ls = []
        ...
        if rank == 0:
            early_stopping(valid_mse, model.module)
