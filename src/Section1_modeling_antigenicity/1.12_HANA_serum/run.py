...
## split train, valid, test data by strains
Identity_cols = ['seq_id_a', 'seq_id_b', 'serumPassCat', 'serumName']
train_data, test_data =split_data_by_strain(Crick_all, Identity_cols, 0.1)
train_data, valid_data = train_test_split(train_data, test_size=1/9, random_state=42)
# train_data, valid_data =split_data_by_strain(train_data, Identity_cols, 1/9)
...
early_stopping = EarlyStopping(patience=10, save_dir='../../../trained_model/1.12/')
...
for epoch in range(epochs):
    ...
    # 将epoch信息写入log.txt
    with open('../../../trained_model/1.12/log.txt', 'a') as f:
