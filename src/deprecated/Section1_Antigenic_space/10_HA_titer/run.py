import sys
sys.path.append('../../')
from fluProfiler_models import fluProfiler_Config, fluProfiler_HA
...
class fluProfiler_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        return self.emb_file_name_a[idx], self.emb_file_name_c[idx], \
...
train_data = pd.read_csv('../1.5_Model_compare/data_back/serum_split/train.csv')
test_data = pd.read_csv('../1.5_Model_compare/data_back/serum_split/test.csv')
...
model = fluProfiler_HA(config=fluProfiler_config, args=fluProfiler_args)
model.to(device)
...
early_stopping = EarlyStopping(patience=10, save_dir='../../../trained_model/1.10/')
...
for epoch in range(epochs):
    ...
    for batch in train_dataloader:
        ...
        loss, logits, output = model(matrices_a=matrixs_a, matrices_b=matrixs_c, 
                                     matrix_attention_masks_a=masks_a, matrix_attention_masks_b=masks_c,
    ...
    # 将epoch信息写入log.txt
    with open('../../../trained_model/1.10/log.txt', 'a') as f:
