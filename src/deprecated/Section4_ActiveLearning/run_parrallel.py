...
class fluProfiler_Dataset(Dataset):
    def __init__(self, DataFrame):
        ...
        self.institute = convert_Institute2tensor(DataFrame['institute'].tolist())
    ...
    def __getitem__(self, idx):
        return self.emb_file_name_a[idx], self.emb_file_name_b[idx], self.emb_file_name_c[idx], self.emb_file_name_d[idx], \
               self.strainPassCats[idx], self.institute[idx], self.labels[idx]
...
def convert_Institute2tensor(institutes):
    result = [
        item.replace('<Crick>', '0').replace('<CDC>', '1').replace('<CNIC>', '2')
...
train_data = pd.read_csv('/mnt/zzbnew/peixunban/chenyihao/fluProfiler_source/data/data_40/fluProfiler_ultra/train.csv', index_col=False)
valid_data = pd.read_csv('/mnt/zzbnew/peixunban/chenyihao/fluProfiler_source/data/data_40/fluProfiler_ultra/valid.csv', index_col=False)
test_data = pd.read_csv('/mnt/zzbnew/peixunban/chenyihao/fluProfiler_source/data/data_40/fluProfiler_ultra/test.csv', index_col=False)
Artificial_data = pd.read_csv('/mnt/zzbnew/peixunban/chenyihao/fluProfiler_source/data/data_40/fluProfiler_ultra/Artificial_data.csv', index_col=False)
train_data = pd.concat([train_data, Artificial_data])
...
agg_dict['label'] = 'first'  ## keep the real sample instead of artificial sample
train_data_final = train_data.groupby(group_columns).agg(agg_dict).reset_index()

# test_sample = 1000
# train_data_final = train_data_final.iloc[:test_sample,]
# valid_data = valid_data.iloc[:test_sample,]
# test_data = test_data.iloc[:test_sample,]
...
emb_df = pd.concat([train_data_final, valid_data, test_data], axis=0)
...
emb_dict = load_embedding("/mnt/zzbnew/peixunban/chenyihao/fluProfiler_source/data/data_40/fluProfiler_ultra/embedding", files=sequence_names)
...
epochs = 200
num_training_steps = len(train_data_final) * epochs

progress_bar = tqdm(range(num_training_steps))
early_stopping = EarlyStopping(patience=20, save_dir='../../trained_model/fluProfiler_ultra/')

for epoch in range(epochs):
    epoch_train_data = train_data_final.sample(frac=1, random_state=42 + epoch).reset_index(drop=True)
    train_dataset = fluProfiler_Dataset(epoch_train_data)
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=False)

    model.train()
    loss_ls = []
    for i, batch in enumerate(train_dataloader):
        if i%chunk_batches==0:
            sample_start = i*batch_size
            sample_end = (i+chunk_batches)*batch_size if (i+chunk_batches)*batch_size < len(epoch_train_data) else len(epoch_train_data)
            emb_sub_df = epoch_train_data.iloc[(i*batch_size):(i+chunk_batches)*batch_size,:]
            chunk_sequences = pd.concat([emb_sub_df['seq_id_a'], emb_sub_df['seq_id_b'], emb_sub_df['seq_id_c'], emb_sub_df['seq_id_d']]).unique().tolist()
            chunk_sequences = ['matrix_' + item for item in chunk_sequences]
            emb_dict_chunk = {key: emb_dict[key].to(device) for key, value in emb_dict.items() if key in chunk_sequences}
            # 清除不必要的变量并释放显存
            del emb_sub_df
            torch.cuda.empty_cache()
    
        emb_file_name_a, emb_file_name_b, emb_file_name_c, emb_file_name_d, strainPassCats, institute, labels = batch

        matrixs_a, masks_a = generate_matrix([emb_dict_chunk[key] for key in emb_file_name_a])
        matrixs_b, masks_b = generate_matrix([emb_dict_chunk[key] for key in emb_file_name_b])
        matrixs_c, masks_c = generate_matrix([emb_dict_chunk[key] for key in emb_file_name_c])
        matrixs_d, masks_d = generate_matrix([emb_dict_chunk[key] for key in emb_file_name_d])
        ...
        loss, logits, output = model(matrices_a=matrixs_a, matrices_b=matrixs_b, matrices_c=matrixs_c, matrices_d=matrixs_d,
                                     ...
                                     strainPassCats=strainPassCats, institute=institute, labels=labels)
    ...
    for batch in valid_dataloader:
        ...
        with torch.no_grad():
            loss, logits, output = model(matrices_a=matrixs_a, matrices_b=matrixs_b, matrices_c=matrixs_c,
                                         ...
                                         institute=institute, labels=labels)
    ...
    # 将epoch信息写入log.txt
    with open('../../trained_model/fluProfiler_ultra/log.txt', 'a') as f:
