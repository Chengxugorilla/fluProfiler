...
DNA_data_filt1 = train_data.groupby(['serumHA_DNA', 'virusHA_DNA', 'serumPassCat', 'virusPassCat']) \
        .agg({'serumName': 'first', 'virusName': 'first', 'Type': 'first', 'label': 'mean'}) \
        .reset_index()
## remove PassCat = 'BOTH'
DNA_data_filt2 = DNA_data_filt1[(DNA_data_filt1['serumPassCat'] != 'BOTH') &
                              (DNA_data_filt1['virusPassCat'] != 'BOTH')].reset_index(drop=True)
## replace PassCat to special token
DNA_data_filt3 = DNA_data_filt2.replace({'serumPassCat': {'EGG': '<EGG>', 'CELL': '<CELL>'},
                                       'virusPassCat': {'EGG': '<EGG>', 'CELL': '<CELL>'}})

train_data_final = DNA_data_filt3.copy()

train_dataset = DNADataset(train_data_final)
valid_dataset = DNADataset(valid_data)
...
early_stopping = EarlyStopping(patience=20, save_dir='/mnt/zzbnew/peixunban/chenyihao/fluProfiler_source/trained_model/1.1_DNA_model/')
# Training loop
for epoch in range(num_epochs):
    ...
    ## 将epoch信息写入log.txt
    with open('/mnt/zzbnew/peixunban/chenyihao/fluProfiler_source/trained_model/1.1_DNA_model/log.txt', 'a') as f:
