...
class HANA_withoutPass_Dataset(Dataset):
    def __init__(self, DataFrame):
        self.sequence = (DataFrame['seq_a'] + '<eos>' + DataFrame['seq_b'] + '<eos>' + DataFrame['seq_c'] + '<eos>' + DataFrame['seq_d']).tolist()
...
save_dir = '/mnt/zzbnew/peixunban/chenyihao/fluProfiler_source/trained_model/1.4_withoutPass_model/'
...
HANA_data_filt1 = train_data.groupby(['seq_a', 'seq_b', 'seq_c', 'seq_d']) \
        .agg({'serumName': 'first', 'virusName': 'first', 'Type': 'first', 'label': 'mean'}) \
        .reset_index()

train_data_final = HANA_data_filt1.copy()

with open(save_dir + 'log.txt', 'a') as f:
    f.write(f"[trian data size: {len(train_data_final)}, valid data size: {len(valid_data)}, test data size: {len(test_data)}]\n")

train_dataset = HANA_withoutPass_Dataset(train_data_final)
valid_dataset = HANA_withoutPass_Dataset(valid_data)
test_dataset = HANA_withoutPass_Dataset(test_data)
...
tokenizer = BioTokenizer(vocab_file='./vocab_AA_withoutPass.txt')
