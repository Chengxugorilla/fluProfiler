"""Plot pooled H3N2 seasonal label-versus-prediction scatters for three models."""
...
OUTPUT_DIR = PROJECT_ROOT / "paper/data/H3N2_season_prediction_scatters_without_name"
SEASONS = ("39", "40", "41", "42", "43", "44")
MODELS = ("SCMS-FiLM-H3", "Nextflu", "Adaboost")
COLORS = {"SCMS-FiLM-H3": "#4C78A8", "Nextflu": "#B79A20", "Adaboost": "#59A14F"}


def load_predictions(model: str, season: str) -> pd.DataFrame:
    if model == "Nextflu":
        path = RESULTS_ROOT / "Nextflu/season" / season / "H3N2/predictions.csv"
        ...
        path = RESULTS_ROOT / "Adaboost/season" / season / "H3N2/predictions.csv"
        frame = pd.read_csv(path)
        frame = frame.loc[
            (frame["Type"] == "H3N2")
            & (frame["split"] == "test")
        ...
        path = RESULTS_ROOT / "SCMS-FiLM-H3/season" / season / "subtype/H3N2_seed42/predictions_test.csv"
        frame = pd.read_csv(path)
        frame = frame.loc[frame["Type"] == "H3N2", ["label", "mean"]].rename(columns={"mean": "prediction"})
    else:
...
def plot_model(model: str) -> None:
    frame = pd.concat([load_predictions(model, season) for season in SEASONS], ignore_index=True)
    ...
    low, high = float(min(labels.min(), predictions.min())), float(max(labels.max(), predictions.max()))
    ...
    ax.set_title(f"H3N2 seasonal split: {model}")
    ax.legend(frameon=False, loc="upper left")
    ax.text(0.98, 0.04, f"n = {len(frame):,}\nslope = {slope:.3f}\nPearson r = {pearson_r:.3f}\nMAE = {mae:.3f}", transform=ax.transAxes, ha="right", va="bottom", bbox={"boxstyle": "round,pad=0.35", "facecolor": "white", "edgecolor": "#BBBBBB", "alpha": 0.9})
