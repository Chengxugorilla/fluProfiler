"""
HANA-only v2 training entrypoint.

This script mirrors legacy HANA data flow (a/b/c/d) while supporting
the unified v2 model I/O contract.
"""
...
def load_dataloaders_and_embedding_keys(
    ...
    train_dataset = HANADataset(train_data_final, add_special_token=add_special_token)
    valid_dataset = HANADataset(valid_data, add_special_token=add_special_token)
    test_dataset = HANADataset(test_data, add_special_token=add_special_token)
    ...
    sequence_names = pd.concat(
        [embedding_df["seq_id_a"], embedding_df["seq_id_b"], embedding_df["seq_id_c"], embedding_df["seq_id_d"]]
    ).unique().tolist()
...
def _batch_to_tensors(batch, device, cache):
    emb_a, emb_b, emb_c, emb_d, strain, labels = batch
    m_a, mk_a = generate_matrix_on_device([cache.get(k) for k in emb_a], device=device)
    m_b, mk_b = generate_matrix_on_device([cache.get(k) for k in emb_b], device=device)
    m_c, mk_c = generate_matrix_on_device([cache.get(k) for k in emb_c], device=device)
    m_d, mk_d = generate_matrix_on_device([cache.get(k) for k in emb_d], device=device)
    return m_a, mk_a, m_b, mk_b, m_c, mk_c, m_d, mk_d, strain.to(device), labels.to(device)


def train_step(model, batch, device, cache, model_impl: str):
    m_a, mk_a, m_b, mk_b, m_c, mk_c, m_d, mk_d, strain, labels = _batch_to_tensors(batch, device, cache)
    if model_impl == "v2":
        out = model(
            BatchInput(
                matrices={
...
def parse_args():
    parser = argparse.ArgumentParser(description="HANA v2 trainer (supports CLI overrides)")
    parser.add_argument("cfg_path", type=str, nargs="?", default=str(_THIS_FILE.parent / "config_v2_hana.json"))
...
def main():
    ...
    run_tag = cfg["experiment"].get("run_tag", "HANA_v2")
    ...
    if not exp_id:
        ...
        exp_id = f"HANA/{_season}/{model_impl}"
    ...
    if model_impl == "v2":
        ...
        model = fluProfiler_HANA(config=flu_config, args=flu_args)
