...
def generate_matrix_on_device(matrix_list, device=None):
    ...
    if len(matrix_list) == 0:
        logging.error("matrix_list is empty.")
        raise ValueError("matrix_list is empty.")

    # 打印 matrix_list 的长度
    logging.debug(f"Number of matrices: {len(matrix_list)}")
    ...
    # 打印第一个矩阵的形状和设备
    logging.debug(f"First matrix shape: {first.shape}, device: {first.device}")
    ...
    # 打印 lengths, max_len, batch_size, embed_dim
    logging.debug(f"Lengths: {lengths}, max_len: {max_len}, batch_size: {batch_size}, embed_dim: {embed_dim}")
    ...
    for i, (mat, L) in enumerate(zip(matrix_list, lengths)):
        # 打印每个矩阵的形状和长度（仅在前几个批次输出）
        if i < 10:  # 仅输出前10个矩阵的信息
            logging.debug(f"Matrix {i} shape: {mat.shape}, length: {L.item()}")
            if mat.shape[0] != L.item():
                logging.warning(f"Mismatch between matrix shape and length for matrix {i}: shape={mat.shape[0]}, length={L.item()}")
