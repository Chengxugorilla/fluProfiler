...
MODEL_CLASS = fluProfiler_v3_1  # 使用的模型类
tag = os.environ.get("FLUPROFILER_TAG") or "v3_1_cached"
batch_size = 64
gpu_cache_GB = 24
lr = 0.00008
device = torch.device('cuda:5')

# 保存路径不再根据脚本路径推断：固定写到 runs/reverse_tests/<season>/<model_version>/<run_id>/
_season = season_path.strip("/").split("test_")[-1]
_model_version = MODEL_CLASS.__name__.replace("fluProfiler_", "", 1)
exp_id = os.environ.get("FLUPROFILER_EXP_ID") or f"reverse_tests/{_season}/{_model_version}"
