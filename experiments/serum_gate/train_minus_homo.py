#!/usr/bin/env python3
"""
Train SerumGate-Minus with homologous, heterologous, and distance supervision.
"""
...
DEFAULT_LABEL_COLUMNS = ("label", "homo_label", "diff_label")
...
def resolve_label_weight_thresholds(
    ...
    labels = pd.to_numeric(train_frame[label_col], errors="raise")
...
def validate_frame_columns(
    frame: pd.DataFrame,
    source: str | Path,
    label_columns: tuple[str, str, str] = DEFAULT_LABEL_COLUMNS,
) -> None:
...
def load_fixed_split_frames(
    ...
    for split_name, frame in frames.items():
        validate_frame_columns(frame, data_dir / f"{split_name}.csv", label_columns=label_columns)
    if type_filter:
...
class SerumGateTaskDataset(Dataset):
    def __init__(
        ...
        query_label_col: str = "label",
        homo_label_col: str = "homo_label",
        distance_label_col: str = "diff_label",
    ):
        ...
        self.query_label_col = query_label_col
        self.homo_label_col = homo_label_col
        self.distance_label_col = distance_label_col
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        ...
        query_labels = torch.tensor(task_frame[self.query_label_col].to_numpy(dtype=float), dtype=torch.float32)
        homo_labels = torch.tensor(task_frame[self.homo_label_col].to_numpy(dtype=float), dtype=torch.float32)
        distance_labels = torch.tensor(task_frame[self.distance_label_col].to_numpy(dtype=float), dtype=torch.float32)
...
def serum_regression_metrics(prediction_frame: pd.DataFrame) -> dict[str, float]:
    if prediction_frame.empty:
        return {key: 0.0 for key in ("pooled_mae", "pooled_mse", "pooled_pearson", "pooled_spearman")}
...
def serum_homo_regression_metrics(prediction_frame: pd.DataFrame) -> dict[str, float]:
    empty_keys = (
        "pooled_mae",
        "pooled_mse",
        "pooled_pearson",
    ...
    diff_y = prediction_frame["diff_label"].to_numpy(dtype=float)
    diff_mean = prediction_frame["mean"].to_numpy(dtype=float)
    homo_y = prediction_frame["homo_label"].to_numpy(dtype=float)
    homo_mean = prediction_frame["self_score"].to_numpy(dtype=float)
    query_y = prediction_frame["label"].to_numpy(dtype=float)
    query_mean = prediction_frame["query_score"].to_numpy(dtype=float)
    log_var = prediction_frame["log_var"].to_numpy(dtype=float)

    diff = diff_mean - diff_y
    homo_diff = homo_mean - homo_y
    query_diff = query_mean - query_y
    var = np.exp(np.clip(log_var, -12.0, 12.0))
    nll = 0.5 * (log_var + diff**2 / var)
    sigma = np.sqrt(var)
    ...
    within_spearman = []
    for _, group in prediction_frame.groupby("task_key"):
        gy = group["diff_label"].to_numpy(dtype=float)
        gm = group["mean"].to_numpy(dtype=float)
        gd = gm - gy
        per_serum_mae.append(float(np.mean(np.abs(gd))))
        serum_bias.append(float(np.mean(gd)))
    ...
    return {
        "pooled_mae": diff_mae,
        "pooled_mse": diff_mse,
        "pooled_pearson": _corr(diff_y, diff_mean),
        "pooled_spearman": _spearman(diff_y, diff_mean),
        "diff_mae": diff_mae,
        "diff_mse": diff_mse,
        "diff_pearson": _corr(diff_y, diff_mean),
        "diff_spearman": _spearman(diff_y, diff_mean),
        "homo_mae": float(np.mean(np.abs(homo_diff))),
        "homo_mse": float(np.mean(homo_diff**2)),
        "homo_pearson": _corr(homo_y, homo_mean),
        "homo_spearman": _spearman(homo_y, homo_mean),
        "query_mae": float(np.mean(np.abs(query_diff))),
        "query_mse": float(np.mean(query_diff**2)),
        "query_pearson": _corr(query_y, query_mean),
        "query_spearman": _spearman(query_y, query_mean),
        "per_serum_mae_mean": float(np.mean(per_serum_mae)),
        "per_serum_mae_median": float(np.median(per_serum_mae)),
        "within_serum_pearson_mean": float(np.mean(within_pearson)) if within_pearson else 0.0,
        "within_serum_spearman_mean": float(np.mean(within_spearman)) if within_spearman else 0.0,
    ...
    for key, value in epoch_metrics.items():
        if isinstance(value, dict):
            for metric_name, metric_value in value.items():
                row[f"{key}_{metric_name}"] = round_float_value(metric_value, decimals)
        else:
            row[key] = round_float_value(value, decimals)
    return row
...
def build_loader(
    ...
    dataset = SerumGateTaskDataset(
        frame,
        vocabs,
        task_cols=task_cols,
        max_queries_per_task=max_queries_per_task,
        query_label_col=query_label_col,
        homo_label_col=homo_label_col,
        distance_label_col=distance_label_col,
    )
...
def masked_mean(values: torch.Tensor, mask: torch.Tensor | None) -> torch.Tensor:
    if mask is None:
        return values.mean()
    mask = mask.float().to(values.device)
    return (values * mask).sum() / mask.sum().clamp_min(1.0)


def item_labels(item: dict[str, Any], name: str, device: torch.device) -> torch.Tensor:
    return item[name].view(1, -1).float().to(device)


def score_supervision_loss(
    ...
    if loss_name == "nll":
        ...
        per_value = 0.5 * (log_var + (target - prediction).pow(2) * torch.exp(-log_var))
    else:
        per_value = F.smooth_l1_loss(prediction, target, beta=0.5, reduction="none")
    return masked_mean(per_value, mask)
...
def multitask_loss(
    ...
    distance_loss = out["nll_loss"] if loss_name == "nll" else out["huber_loss"]
    homo_loss = score_supervision_loss(
        out["self_score"],
        item_labels(item, "homo_labels", device),
        out["self_log_var"],
        batch.query_mask,
        loss_name,
    )
    query_loss = score_supervision_loss(
        out["query_score"],
        item_labels(item, "query_labels", device),
        out["query_log_var"],
        batch.query_mask,
        loss_name,
    )
    loss = (
        float(distance_loss_weight) * distance_loss
        + float(homo_loss_weight) * homo_loss
        + float(query_loss_weight) * query_loss
    )
    return loss, {
        "distance_loss": float(distance_loss.detach().item()),
        "homo_loss": float(homo_loss.detach().item()),
...
def evaluate_model(
    ...
    with torch.no_grad():
        for batch, items in loader:
            ...
            out = model(batch)
            loss, _loss_parts = multitask_loss(
                out,
                batch,
                item,
                loss_name,
            ...
            distance_labels = item["distance_labels"].detach().cpu().view(-1).numpy()
            homo_labels = item["homo_labels"].detach().cpu().view(-1).numpy()
            query_labels = item["query_labels"].detach().cpu().view(-1).numpy()
            meta = item["query_meta"]
            for idx in range(len(distance_labels)):
                row = meta.iloc[idx].to_dict()
                row.update(
                    {
                        "task_key": item["task_key"],
                        "label": float(query_labels[idx]),
                        "homo_label": float(homo_labels[idx]),
                        "diff_label": float(distance_labels[idx]),
                        "mean": float(mean[idx]),
    ...
    metrics = serum_homo_regression_metrics(pred_frame)
...
def run_training(args: argparse.Namespace) -> dict[str, Any]:
    ...
    label_columns = (args.query_label_col, args.homo_label_col, args.distance_label_col)
    ...
    with tqdm(
        ...
        for epoch in range(args.epochs):
            ...
            if args.refit_train_valid:
                valid_metrics = serum_homo_regression_metrics(pd.DataFrame())
            ...
            if args.skip_test_eval:
                test_metrics = serum_homo_regression_metrics(pd.DataFrame())
            ...
            if args.best_metric not in valid_metrics:
                ...
                raise ValueError(f"--best-metric {args.best_metric!r} not found in validation metrics: {available}")
            is_refit_final_epoch = args.refit_train_valid and epoch + 1 == args.epochs
            is_best_valid_epoch = (not args.refit_train_valid) and valid_metrics[args.best_metric] < best_valid
            ...
            if (
                ...
                with paths["log"].open("a", encoding="utf-8") as handle:
                    handle.write(
                        ...
                        f"no valid_{args.best_metric} improvement for {epochs_without_improvement} epoch(s).\\n"
                    )
                break
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train SerumGate-Minus-Homo.")
    ...
    parser.add_argument(
        ...
        type=str,
        default="label",
        help="Column containing heterologous query titers.",
    )
    parser.add_argument(
        "--homo-label-col",
        type=str,
        default="homo_label",
        help="Column containing homologous serum titers.",
    )
    parser.add_argument(
        "--distance-label-col",
        type=str,
        default="diff_label",
        help="Column containing antigenic distance targets, usually homo_label - label.",
    )
    parser.add_argument(
        "--refit-train-valid",
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Weight for distance supervision on self_score - query_score.",
    )
    parser.add_argument(
        "--homo-loss-weight",
        type=float,
        default=1.0,
        help="Weight for homologous titer supervision on self_score.",
    )
    parser.add_argument(
        "--query-loss-weight",
        type=float,
        default=1.0,
        help="Weight for heterologous titer supervision on query_score.",
    )
    parser.add_argument(
        "--best-metric",
        type=str,
    ...
    if min(args.distance_loss_weight, args.homo_loss_weight, args.query_loss_weight) < 0:
        parser.error("loss weights must be non-negative")
    if args.distance_loss_weight + args.homo_loss_weight + args.query_loss_weight <= 0:
