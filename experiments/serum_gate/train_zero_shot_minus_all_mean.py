"""Train the HA-mean, no-NA SerumGate-Minus variant.

The data pipeline, optimization, metrics, early stopping, and checkpoint format
are reused unchanged from ``train_zero_shot_minus.py``. Only the model class is
replaced, with HA mean pooling and the NA branch forced by the model variant.
"""
...
def _load_whole_csv_frames(
    ...
    """Load whole.csv as the sole training split."""
...
def run_training(args: argparse.Namespace) -> dict[str, Any]:
    ...
    training_args.ha_pooling = "mean"
    training_args.na_branch = "none"
    ...
    base_trainer.SerumGateMinusModel = SerumGateMinusAllMeanModel
    try:
